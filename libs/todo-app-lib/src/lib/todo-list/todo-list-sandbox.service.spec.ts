...
describe('Service: TodoListSandboxService', () => {
	...
	todoListResourcesServiceStub.addTodo.and.returnValue(
		of(new TODOItem('', '')),
	);
	todoListResourcesServiceStub.updateTodo.and.returnValue(
		of(new TODOItem('', '')),
	);
			...
			service
				...
				.subscribe(() => {
					expect(store.dispatch).toHaveBeenCalledWith(
						new SaveTodoItemStartedAction(),
					);
					expect(store.dispatch).toHaveBeenCalledWith(
						new AddTodoItemSuccessAction(todoItem),
					);
					done();
				});
		});
		...
		it('should update todo', done => {
			...
			service
				...
				.subscribe(() => {
					...
					expect(store.dispatch).toHaveBeenCalledWith(
						new UpdateTodoItemSuccessAction(todoItem),
					);
					done();
				});
		});
